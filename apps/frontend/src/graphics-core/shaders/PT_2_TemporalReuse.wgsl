...
//  - 현재/이전 후보 중 하나만 선택 (UCW는 선택한 쪽 값을 그대로 유지)
//==========================================================================
...
fn UpdateReservoirTemporal(
    ...
    let curRes  : Reservoir = ReservoirBuffer[idx];
    let curUCW  : f32 = curRes.UCW;
    let prevUCW : f32 = prevRes.UCW;

    // 기본값: 현재 리저버 그대로
    var result : Reservoir = curRes;

    // 이전 샘플 없으면 현재만 사용
    if (prevUCW <= 0.0) {
        ReservoirBuffer[idx] = result;
        return;
    }

    // 현재 샘플 없으면 이전 것 그대로
    if (curUCW <= 0.0) {
        ReservoirBuffer[idx] = prevRes;
        return;
    }

    // 둘 다 유효 → 리저버 샘플링
    var w_cur  : f32 = curUCW * curRes.P_hat;
    var w_prev : f32 = prevUCW * (prevRes.P_hat);

    let w_sum   : f32 = w_cur + w_prev;
    let p_prev : f32 = w_prev / w_sum;

var seed : u32 = GetHashValue(
    curPixel.x * 374761393u +
    curPixel.y * 668265263u +
    UniformBuffer.FrameIndex * 362437u +
    u32(curRes.C) * 521u
);
    ...
    if (r < p_prev) {
        result = prevRes;
        result.UCW = w_sum/prevRes.P_hat;
    } else {
        result = curRes;
        result.UCW = w_sum/curRes.P_hat;
    }

    // 중요: UCW와 C는 누적
    
    result.C   = min(curRes.C + prevRes.C, 255u);
...
fn cs_main(@builtin(global_invocation_id) ThreadID: vec3<u32>)
{
    ...
    var prevRes : Reservoir = Reservoir();

    // 2. 이전 프레임 리저버 읽기 (reprojection 성공한 경우에만)
    if (all(prevPixel >= vec2<i32>(0, 0)) &&
        ...
        prevRes = PrevReservoirBuffer[prevIdx];
    }
    
        UpdateReservoirTemporal(curPixel, prevRes);

    // 3. 현재 프레임 리저버 + 이전 프레임 후보(prevRes)를 머지
