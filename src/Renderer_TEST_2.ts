...
import ShaderCode_Pass2             from './shaders/ReSTIR_DI_Pass2_fixed.wgsl?raw';
import ShaderCode_Pass3             from './shaders/ReSTIR_DI_Pass3.wgsl?raw';
import ShaderCode_Pass4             from './shaders/ReSTIR_DI_Pass4.wgsl?raw';
...
export class RendererTEST
{
    ...
    public Update() : void
    {
        ...
        const UniformData = new ArrayBuffer(4 * ELEMENT_COUNT);
        {
            ...
            Uint32View[32] = this.World.Lights.length;
             ...
             for(let iter=0; iter<16; iter++) Float32View[33 + iter] = prevVP[iter]!;
    ...
    public Render() : void
    {
        ...
        CommandEncoder.copyTextureToTexture(
            ...
            {
                texture : this.GPUTextures[ETextureIndex.Reservoir_Prev],
            },
            {
    ...
    private SerializeWorldData() : [ArrayBuffer, ArrayBuffer, ArrayBuffer, ImageBitmap[]]
    {
        ...
        let LightsCDFOffsetData    : Uint32Array;
        {
            let LightCDFRawData = this.World.GetLightCDFBuffer();
            
            const LightsCDFArrayBuffer : ArrayBuffer = new ArrayBuffer(4 * LightCDFRawData.length);
            

            const Float32View : Float32Array = new Float32Array(LightsCDFArrayBuffer);

            for(let cdfID = 0; cdfID<LightCDFRawData.length; cdfID++){
                Float32View[cdfID] = LightCDFRawData[cdfID];
    ...
    private CreateGPUResources() : void
    {
        ...
        this.GPUTextures[ETextureIndex.Reservoir_Prev] = this.CreateGPUTexture(GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_SRC | GPUTextureUsage.COPY_DST);
        this.GPUTextures[ETextureIndex.Reservoir_Read] = this.CreateGPUTexture(GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_SRC | GPUTextureUsage.COPY_DST);
    ...
    private async CreateComputePasses() : Promise<void>
    {
        ...
        [
            ...
            (
                ...
                [   // Output, GPUTextureView
                    this.GPUTextures[ETextureIndex.Reservoir_Read].createView(),
                ]
            ),
            ...
            (
                ...
                [   // Input, GPUTextureView
                    this.GPUTextures[ETextureIndex.G_Buffer].createView(),
                    this.GPUTextures[ETextureIndex.Reservoir_Prev].createView(),
                    this.GPUTextures[ETextureIndex.Reservoir_Read].createView(),
                ],
                [   // Output, GPUBuffer
    ...
    private CreateRenderPass() : void
    {
        ...
        // Create Render BindGroup
        {
            const RenderBindGroupDescriptor: GPUBindGroupDescriptor =
            {
                ...
                entries : 
                [
                    { binding : 0, resource : this.GPUTextures[ETextureIndex.Reservoir_Read].createView() }
                ],
            };
